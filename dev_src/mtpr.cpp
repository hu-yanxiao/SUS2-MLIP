	    ...
	    {for (int j = 0;j<C; j++ )
	        {for (int n=0; n<200001; n++)
	            {for (int mu=0; mu< radial_func_count;mu++)
	                ...
	                {
	                factor=regression_coeffs[C+2*C*C* K_ + mu * (R+C) + xi] *regression_coeffs[C+2*C*C* K_ + mu * (R+C)+R + i] * regression_coeffs[C+2*C*C* K_ +mu * (R+C)+ R + j];
...
//		int type_outer = Neighborhood.types[j];/
	...
	//		for (int xi = 0; xi < p_RadialBasis->rb_size; xi++)
		...
		for (int i = 0; i < alpha_index_basic_count; i++) {
			...
			for (int xi = 0; xi < p_RadialBasis->rb_size; xi++)
				val += regression_coeffs[C+2*C*C*K_+ mu * (R+C) + xi] *regression_coeffs[C+2*C*C*K_+mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C*K_+mu * (R+C)+ R + type_outer]* val_[k_*R+xi];
	...
	//	int type_outer = Neighborhood.types[j];
		...
		for (int i = 0; i < alpha_index_basic_count; i++) {
			...
			for (int xi = 0; xi < p_RadialBasis->rb_size; xi++)
			{
				...
				val += regression_coeffs[C+2*C*C* K_ + mu * (R+C) + xi] *regression_coeffs[C+2*C*C* K_ +mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C* K_ +mu * (R+C)+ R + type_outer] * val_[k_*R+xi];
				der += regression_coeffs[C+2*C*C* K_ + mu * (R+C) + xi] *regression_coeffs[C+2*C*C* K_ +mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C* K_ +mu * (R+C)+ R + type_outer] * der_[k_*R+xi];
...
//		int type_outer = nbh.types[j];
		...
		for (int i = 0; i < alpha_index_basic_count; i++) {
			...
			{
				val += regression_coeffs[C+2*C*C*K_+ mu * (R+C) + xi] *regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_outer]* val_[k_*R+xi];
				der += regression_coeffs[C+2*C*C * K_ + mu * (R+C) + xi] *regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_outer] * der_[k_*R*5+xi];
		...
		//	int type_outer = nbh.types[j];
			...
			for (int i = 0; i < alpha_index_basic_count; i++) {
				...
				for (int xi = 0; xi < R; xi++)
				{
					...
					mom_val[i] += regression_coeffs[C+2*C*C * K_ + mu * (R+C) + xi] *regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C * K_ +mu * (R+C)+ R + type_outer] * val_[k_*R+xi] * powk*mult0;
			...
			for (int j = 0; j < C; j++)
				...
				{  for (int m = 0; m < R; m++) {
					out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+m] += linear_coeffs[nbh.my_type]*dloss_dmom[i] * mom_rad_jacobian_(i, j, l*R+m)*
                                        regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + j]  ; //from basic moments
					//out_grad_accumulator[(C * j + type_central) * R * K + k] += 0.5*dloss_dmom[i] * mom_rad_jacobian_(i, j, k);
                                        out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+R+type_central] += linear_coeffs[nbh.my_type] * dloss_dmom[i] * mom_rad_jacobian_(i, j, l*R+m)*regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + j]
                                        *regression_coeffs[C+2*C*C * K_ +l * (R+C)+ m]  ;
                                        out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+R+j] += linear_coeffs[nbh.my_type] * dloss_dmom[i] * mom_rad_jacobian_(i, j, l*R+m)*regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + type_central]
                                        *regression_coeffs[C+2*C*C * K_ +l * (R+C)+ m]  ;
					...
					out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+m] += linear_coeffs[nbh.my_type] * site_energy_ders_wrt_moments_[i] * mom_rad_coord_jacobian_(i, j, l*R+m)
                                        *regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + type_central] * regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + j]    ; //from basic moment ders
                                        out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+R+type_central] += linear_coeffs[nbh.my_type] * site_energy_ders_wrt_moments_[i] * mom_rad_coord_jacobian_(i, j, l*R+m)
                                        *regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + j]*regression_coeffs[C+2*C*C * K_ +l * (R+C)+ m] ;
                                        out_grad_accumulator[C+2*C*C * K_ +l*(R+C)+R+j] += linear_coeffs[nbh.my_type] * site_energy_ders_wrt_moments_[i] * mom_rad_coord_jacobian_(i, j, l*R+m)
                                        *regression_coeffs[C+2*C*C * K_ +l * (R+C)+ R + type_central ]*regression_coeffs[C+2*C*C * K_ +l * (R+C)+ m] ;
