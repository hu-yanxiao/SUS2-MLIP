...
void RadialBasis_Chebyshev_ssss::RB_Calc(double r, double scal, double s)
{
	...
	double cutoff = 1 - (p + 1) * (p + 2) * pow(r_d, p) * 0.5 + p * (p + 2) * pow(r_d, p + 1) - (p + 1) * p * pow(r_d, p + 2) * 0.5;
	double cutoff_der = p * (p + 1) * (p + 2) * (pow(r_d, p) - 0.5 * (pow(r_d, p + 1) + pow(r_d, p - 1))) / max_dist;


	rb_vals[0] = scaling * (1 * cutoff);
	rb_ders[0] = scaling * (0 * cutoff + cutoff_der);
	rb_ders[0 + rb_size * 1] = scaling * 0;
	rb_ders[0 + rb_size * 2] = scaling * 0;
	rb_ders[0 + rb_size * 3] = scaling * 0;
	rb_ders[0 + rb_size * 4] = scaling * 0;
	rb_vals[1] = scaling * (ksi * cutoff);
	rb_ders[1] = scaling * (mult * cutoff +  ksi * cutoff_der);
	rb_ders[1 + rb_size * 1] = scaling * mult_scal * cutoff;
	rb_ders[1 + rb_size * 2] = scaling * (mult_scal_r * cutoff +  mult_scal * cutoff_der);
	rb_ders[1 + rb_size * 3] = scaling * mult_s * cutoff;
	rb_ders[1 + rb_size * 4] = scaling * (mult_s_r * cutoff +  mult_s * cutoff_der);
